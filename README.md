...
* `setup/install.sh`: download e unpack required build/runtime tools
* `setup/build.sh`: deploy all components to folder `mods`
...
IMPORTANT: all scripts assumes they are executed from repo root (pwd)


**setup/install.sh**

Download e unpack required build/runtime tools:
...
deploy all components to folder `mods`:

1. Remove all modules in folder `mods`
...
* `gradle mongoDbStart` - starts 3 instance of MongoDB
* `gradle mongoDbStop` - stops the instances of MongoDB
...
* `data/*.log`: 3 log files, one for each instance (* = 1,2,3)
...
* `web.sh`: starts application, background, creates `web.pid` e `web.log`
* ``kill `cat web.pid` ``: ends application 
* `tail -f web.log`: shows application output

Modules:

* `cavani.endorfina-hydrogen.web-v1.0`: HTTP Server (static files) and service routing (URL, parameters parsing, forwards request)
* `cavani.endorfina-hydrogen.service-v1.0`: Tracker service, updates primary persistence, reads from secondary persistence
...
* `dash.sh`: starts application, background, creates `dash.pid` e `dash.log`
* ``kill `cat dash.pid` ``: ends application 
* `tail -f dash.log`: shows application output

Modules:

* `cavani.endorfina-hydrogen.dashboard-v1.0`: HTTP Server (static files) e service routing (URL, parameters parsing, forwards request)
* `cavani.endorfina-hydrogen.engine-v1.0`: Data services, reads from secondary persistence
* `vertx.mongo-persistor-v1.2.1`: persistence service for MongoDB
